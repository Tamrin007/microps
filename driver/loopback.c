...
static int loopback_isr(unsigned int irq, void *id)
{
    ...
    while (1) {
        ...
        debugf("queue popped (num:%u), dev=%s, type=0x%04x, len=%zd", PRIV(dev)->queue.num, dev->name, dev->type, entry->type, entry->len);
...
struct net_device *loopback_init(void)
{
    ...
    // Exercise 3-1: デバイスの生成とパラメーターの設定
    ...
    // Exercise 3-2: デバイスの登録と割込みハンドラの設定
