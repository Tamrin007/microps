...
int net_device_add_iface(struct net_device *dev, struct net_iface *iface)
{
    ...
    dev->ifaces = iface;
    infof("added interface, dev=%s, family=%d", dev->name, iface->family);
...
int net_input_handler(uint16_t type, const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    for (proto = protocols; proto; proto = proto->next) {
        if (proto->type == type) {
            ...
            debugf("queue pushed (num:%u), dev=%s, type0x%04x, len=%zu", proto->queue.num, dev->name, type, len);
            debugdump(data, len);
...
int net_softirq_handler(void)
{
    ...
    for (proto = protocols; proto; proto = proto->next) {
        while (1) {
            ...
            debugf("queue popped (num:%u), dev=%s, type=%04x, len=%zu", proto->queue.num, entry->dev->name, proto->type, entry->len);
